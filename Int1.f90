program mian
    ...
    implicit none
    integer Natm,Nuc_Char(4),La(3),Lb(3)
    real(kind=8) a,b,S,Ki,Nuc,Ra(3),Rb(3),Rc(3,4)
    real(kind=8) BNa,BNb
    ...
    Rb(:) = (/0.0d0,0.0d0,0.0d0/)
    Rc(:,1) = (/1.0d0,1.0d0,1.0d0/)
    Rc(:,2) = (/2.0d0,2.0d0,2.0d0/)
    Rc(:,3) = (/3.0d0,3.0d0,3.0d0/)
    Rc(:,4) = (/4.0d0,4.0d0,4.0d0/)
    
    a = 1.0d0
    b = 1.0d0
    
    ! |a> = BNa*(x-Ra(1))^La(1)*(y-Ra(2))*La(2)*(z-Ra(3))^La(3)*exp(-a*r^2)
    
    call Bas_Normal(a,La(1),La(2),La(3),BNa)
    call Bas_Normal(b,Lb(1),Lb(2),Lb(3),BNb)
    
    call Overlap_Kinetic_GE(a,b,La,Lb,Ra,Rb,S,Ki,"Y")
    write(*,*) "Overlap: <a|b>=",S*BNa*BNb
    write(*,*) "Kinetic: <a|-1/2*D|b>=",Ki*BNa*BNb
    
    call N_E_Attractive_GE(a,b,La(:),Lb(:),Ra(:),Rb(:),Rc(:,:),Natm,Nuc_Char(:),Nuc)
    write(*,*) "Nuc att: sum[<a|-1/rc|b>,{c=1 to Natm}]=",Nuc*BNa*BNb
...
subroutine Overlap_Kinetic_GE(a,b,La,Lb,Ra,Rb,S,Ki,YN)
    ...
    integer La(3),Lb(3)
    integer(kind=8) Ca,Cb,Comb
    real(kind=8),parameter::Pi = 3.141592653589793d0
    real(kind=8) a,b,K,AB,S,Ki,Ix,Iy,Iz,KIx,KIy,KIz
    real(kind=8) I2x,I2y,I2z,I_2x,I_2y,I_2z,IT,DobF
